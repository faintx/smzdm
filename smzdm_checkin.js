...
const { MagicJS } = require('./util/magicJs.js');
...
; (async () => {
  ...
  try {
    ...
    if (!nickName) {
      ...
      sendMsg(scriptName,'', 'Cookie过期已过期，请重新登录!', params)
      magicJS.done();
    }
    else {
      ...
      if (webCheckinErr) {
        ...
        sendMsg(scriptName, '', 'Web端签到异常，查看日志!', params)
      }
      else {
        ...
        sendMsg(title, subTitle, content, params);
      }
    }
  }
  catch (err) {
